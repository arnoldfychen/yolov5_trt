...
cv::Rect get_rect(cv::Mat& img, float bbox[4]) {
    ...
    if (l <0) l = 0;
    if (t <0) t = 0;
    if (r > img.cols) r = img.cols;
    if (b > img.rows) b = img.rows;
...
IPluginV2Layer* addYoLoLayer(INetworkDefinition *network, std::map<std::string, Weights>& weightMap,
  IConvolutionLayer* det0, IConvolutionLayer* det1, IConvolutionLayer* det2,int numClasses)
{
    auto creator = getPluginRegistry()->getPluginCreator("YoloLayer_TRT", "1");
    std::vector<float> anchors_yolo = getAnchors(weightMap);
    PluginField pluginMultidata[4];
    int NetData[4];
    NetData[0] = numClasses;  
